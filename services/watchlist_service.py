...
def add_to_watchlist(user_id, film_id, public=True):
    ...
    # Deduplication — mirrors add_to_collection(): look for an existing
    # (user_id, film_id) entry and refuse to create a second one.
...
def remove_from_watchlist(user_id, film_id):
    ...
    Mirrors remove_from_collection(): look up the (user_id, film_id) entry
    and raise if it isn't there, rather than silently succeeding.
...
def get_watchlist(user_id):
    ...
    Returns:
        list[dict]: List of film dicts with watchlist metadata attached,
            sorted by date added (newest first) to match get_collection().
    """
    entries = (
